...
(function (window) {
    ...
    class TimeSpanSelector extends HTMLElement {
        ...
        static DELETE_STRING = "verwijderen";
...
.invalid + span.too-early:after {
    content: 'Tijdstip ligt in het verleden';
    color: red;
    font-size: smaller;
    font-weight: bold;
}
...
`;
            ...
            plus.setAttribute("aria-label", "Herinneringstijdstip toevoegen");
            plus.className = "plus";
        ...
        addReminder(reminder, uuid) {
            ...
            optionNow.textContent = "onmiddelijk";
            ...
            optionDelete.textContent = "verwijderen";
        ...
        checkValidity() {
            ...
            for (let i = 0; i < inputs.length; i += 2) {
                ...
                if (this._t0.getTime() - dt < now) {
                    ...
                    inputs[i].title = "Tijdstip ligt in het verleden";
                    inputs[i + 1].classList.add("invalid");
                    inputs[i + 1].title = "Tijdstip ligt in het verleden";
                    ok = false;
                }
                else {
    ...
    function onDocumentClicked() {
        ...
        if (abortController) {
            ...
            console.log("Verzenden geannuleerd.");
        }
    }
    ...
    function onCancel(e) {
        ...
        showStatus("Gebeurtenis wordt geannuleerd &hellip;", { withProgressBar: true });
        ...
        fetch(API.ENDPOINT,
            ...
            showStatus(`Verzenden mislukt: ${err}`);
        }).then(res => {
            if (!res)
                return;
            if (res.ok) {
                res.json().then(result => {
                    if (result.success) {
                        if (result.reminders && result.reminders.length > 0) {
                            showStatus(`Verzenden gelukt. ${result.reminders.length} Herinneringen verwijderd.`);
                            setTimeout(hideStatus, CONFIG.STATUS_TIMEOUT_MS);
                        }
                        else {
                            showStatus("Er zijn herinneringen gevonden om te verwijderen.");
                        }
                    }
                    else {
                        showStatus(`Verwijderen van gebeurtenis mislukt: ${result.error}`, { error: true })
                    }
                });
            }
            else {
                res.json().then(json => {
                    showStatus(`Verzenden mislukt met code ${res.status}: ${json.error.message}`, { error: true });
                }).catch(err => console.error("Verzenden mislukt.", err));
            }
        });
        return true;
    }
    ...
    function onSubmit(e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        showStatus("Herinneringen zijn ingesteld &hellip;", { withProgressBar: true });
        const appointment_data = mkAppointmentObject();
        ...
        fetch(API.ENDPOINT,
            ...
            if (res.ok) {
                res.json().then(result => {
                    if (result.success) {
                        showStatus("Verzenden gelukt.");
                        setTimeout(hideStatus, CONFIG.STATUS_TIMEOUT_MS);
                    }
                    else {
                        showStatus(`Instellen van herinneringen mislukt: ${result.error}`, { error: true });
                    }
                    abortController = null;
                });
            }
    ...
    function checkForm() {
        ...
        if (el.date.value !== "" && nowYYYYMMDD > el.date.value) {
            ...
            el.date.nextElementSibling.textContent = "De datum ligt in het verleden.";
            ok = false;
        }
        else {
        ...
        if (nowYYYYMMDD === el.date.value) {
            ...
            if (el.startTime.value !== "" && nowHHmm >= el.startTime.value) {
                ...
                el.startTime.nextElementSibling.textContent = "Het tijdstip ligt in het verleden.";
                ok = false;
            }
            else {
            ...
            if (el.endTime.value !== "" && nowHHmm >= el.endTime.value) {
                ...
                el.endTime.nextElementSibling.textContent = "Het tijdstip ligt in het verleden.";
                ok = false;
            }
            else {
        ...
        if (el.date.value !== "" && el.startTime.value !== "" && el.endTime.value !== ""
            ...
            el.startTime.classList.add("invalid");
            el.endTime.classList.add("invalid");
            el.startTime.nextElementSibling.textContent = "Begin mag niet na einde liggen.";
            ok = false;
        }
        else {
            el.startTime.classList.remove("invalid");
    ...
    function main() {
        console.info("%cHerinneringshelper voor Teams", "font-weight: bold; color: #14315b;");
